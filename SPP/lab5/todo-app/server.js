...
const server = new ApolloServer({
  ...
  context: {
    ...
    async getTodos() {
      console.log(`${Date.now()}: getTodos`);
      await randomSleep();
      return TODOS;
    },

    async addTodo({ name, complete }) {
      console.log(`${Date.now()}: addTodo`);
      await randomSleep();
    ...
    async updateTodo({ id, name, complete }) {
      console.log(`${Date.now()}: updateTodo`);
      await randomSleep();
    ...
    async deleteTodo(id) {
      console.log(`${Date.now()}: deleteTodo`);
      await randomSleep();
...
export function graphqlTodoPlugin() {
  return {
    ...
    async serverStart({ app, config }) {
      ...
      console.log(
        `🚀 GraphQL Dev Server ready at http://localhost:${config.port}${server.graphqlPath}`
      );
    },
  };
}
